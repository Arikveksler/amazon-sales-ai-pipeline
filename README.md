...
The Streamlit dashboard displays outputs generated by the Analyst Crew (insights and visualizations).

**Step 1 — Generate the reports** (requires an OpenAI API key in `.env`):
```bash
python main.py
```
This creates `insights.md` and `data/eda_report.html`.
...
- **Data Preview** — Upload and inspect the raw CSV
- **Insights** — AI-generated business insights from `insights.md`
- **Visualizations** — Interactive EDA report from `data/eda_report.html`
- **Prediction** — Placeholder for future ML model integration
